...
async def upload_video_async(
    ...
    try:
        ...
        # Upload raw video to Infinia
        handler = S3Handler('ddn_infinia', storage_config.local_cache_config)
        raw_object_key = f"media/raw/videos/{datetime.now().strftime('%Y%m%d_%H%M%S')}_{asset_id}_{filename}"
        
        # Basic metadata for raw video
        metadata = {
            'asset_id': asset_id,
            'modality': 'video',
            'filename': filename,
...
def process_video_background(
    ...
    try:
        ...
        logger.info(f"🎬 Processing video {asset_id}")
        
        # Download video from Infinia to temp file
